...
export default function WaitlistPage() {
  ...
  useEffect(() => {
    ...
    // Interpolate through a full sequence with smoothstep easing
    ...
    // ARMS RISE — inhale, arms float up overhead
    ...
    const allPoses: Pose[] = [
      stand,                                    // breathe
      armsHalf, armsUp, armsHalf,               // arms rise
      stand,                                    // transition
      chair, chair, armsUp,                     // chair
      stand,                                    // transition
      war2, war2, war2,                         // warrior II hold
      stand,                                    // transition
      crescent, crescent, armsUp,               // crescent lunge
      stand,                                    // transition
      reachL, stand, reachR, stand,             // lateral reaches
      lungeR, stand, lungeL,                    // lunges
      stand,                                    // transition
      fold, fold, armsHalf,                     // fold & up
      armsUp, armsHalf,                         // close
      stand,                                    // return
    ]
    ...
    // HER flow: different sequence
    const herPoses: Pose[] = [
      stand,
      tree, tree, tree,
      stand,
      armsHalf, armsUp, armsUp, armsHalf,
      stand,
      wide, wide, wide,
      stand,
      reachR, stand, reachL, stand,
      crescent, crescent, armsUp,
      stand,
      chair, chair, armsUp,
      stand,
      fold, fold, armsHalf,
    ...
    const drawFigure = (pose: Pose, cx: number, cy: number, s: number, alpha: number, hasPonytail = false) => {
      ...
      // Head
      ctx.beginPath()
      ctx.arc(head.x, head.y, s * 0.25, 0, Math.PI * 2)
      ctx.stroke()

      // Hair for the girl — two symmetric ponytails
      if (hasPonytail) {
        const r = s * 0.25
        // Right ponytail
        const rStartX = head.x + r * 0.5
        const rStartY = head.y - r * 0.75
        const rCpX = head.x + r * 1.6
        const rCpY = head.y - r * 0.3
        const rEndX = head.x + r * 1.2
        const rEndY = head.y + r * 1.8
        ctx.beginPath()
        ctx.moveTo(rStartX, rStartY)
        ctx.bezierCurveTo(rCpX, rCpY, rCpX, rCpY + r * 0.8, rEndX, rEndY)
        ctx.stroke()
        // Left ponytail (mirrored)
        const lStartX = head.x - r * 0.5
        const lStartY = head.y - r * 0.75
        const lCpX = head.x - r * 1.6
        const lCpY = head.y - r * 0.3
        const lEndX = head.x - r * 1.2
        const lEndY = head.y + r * 1.8
        ctx.beginPath()
        ctx.moveTo(lStartX, lStartY)
        ctx.bezierCurveTo(lCpX, lCpY, lCpX, lCpY + r * 0.8, lEndX, lEndY)
        ctx.stroke()
      }

      // Neck
      ctx.beginPath()
      ctx.moveTo(head.x, head.y + s * 0.25)
    ...
    const draw = () => {
      ...
      const cx = w * 0.85
      const cy = h * 0.78
      const floatY = Math.sin(frame * 0.005) * 2
      ...
      // Motion trail
      if (frame % 8 === 0) {
        trailPoses.push({ pose: [...pose], age: 0 })
        if (trailPoses.length > 6) trailPoses.shift()
      }
      for (const trail of trailPoses) {
        trail.age++
        const trailAlpha = Math.max(0, 0.06 - trail.age * 0.004)
        if (trailAlpha > 0) drawFigure(trail.pose, cx, cy + floatY, figSize, trailAlpha)
      }
      ...
      ctx.shadowBlur = 20
      drawFigure(pose, cx, cy + floatY, figSize, 0.55)
      ctx.restore()

      // === HER — left side ===
      const cx2 = w * 0.15
      const cy2 = h * 0.78
      const floatY2 = Math.sin(frame * 0.006 + 1.5) * 2
      ...
      // Motion trail - her
      if (frame % 8 === 0) {
        trailPoses2.push({ pose: [...pose2], age: 0 })
        if (trailPoses2.length > 6) trailPoses2.shift()
      }
      for (const trail of trailPoses2) {
        trail.age++
        const trailAlpha = Math.max(0, 0.06 - trail.age * 0.004)
        if (trailAlpha > 0) drawFigure(trail.pose, cx2, cy2 + floatY2, figSize, trailAlpha, true)
      }
      ...
      ctx.shadowColor = 'rgba(201, 169, 110, 0.3)'
      ctx.shadowBlur = 20
      drawFigure(pose2, cx2, cy2 + floatY2, figSize, 0.55, true)
      ctx.restore()

      // Gold burst particles from hands & feet — HIM
      const spt = (i: number) => ({ x: cx + pose[i] * figSize, y: cy + floatY + pose[i + 1] * figSize })
      if (prevPose) {
        const endpoints = [10, 14, 18, 22]
        for (const idx of endpoints) {
          const curr = spt(idx)
          const prevX = cx + prevPose[idx] * figSize
          const prevY = cy + floatY + prevPose[idx + 1] * figSize
          const dx = curr.x - prevX
          const dy = curr.y - prevY
          const dist = Math.sqrt(dx * dx + dy * dy)
          if (dist > 0.4) {
            const count = Math.min(Math.floor(dist), 3)
            emitParticles(curr.x, curr.y, dx, dy, count)
          }
        }
      }

      // Gold burst particles from hands & feet — HER
      const spt2 = (i: number) => ({ x: cx2 + pose2[i] * figSize, y: cy2 + floatY2 + pose2[i + 1] * figSize })
      if (prevPose2) {
        const endpoints = [10, 14, 18, 22]
        for (const idx of endpoints) {
          const curr = spt2(idx)
          const prevX = cx2 + prevPose2[idx] * figSize
          const prevY = cy2 + floatY2 + prevPose2[idx + 1] * figSize
          const dx = curr.x - prevX
          const dy = curr.y - prevY
          const dist = Math.sqrt(dx * dx + dy * dy)
          if (dist > 0.4) {
      ...
      // Draw all gold particles
      drawParticles()

      // Sparkles on endpoints — both figures
      const allEndpoints = [
        ...([10, 14, 18, 22] as number[]).map(i => spt(i)),
        ...([10, 14, 18, 22] as number[]).map(i => spt2(i)),
      ]
      for (const ep of allEndpoints) {
        const sa = 0.15 + Math.sin(frame * 0.05 + ep.x) * 0.1
        ctx.globalAlpha = sa
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white selection:bg-amber-500/20">
      ...
      <section ref={ctaSectionRef} className="relative min-h-[600px] flex flex-col items-center justify-center px-6 text-center overflow-hidden">
        ...
        />
        <h2 className="relative z-10 text-4xl md:text-5xl font-bold mb-6">
          Pr&ecirc;t(e) &agrave; jouer le long terme&nbsp;?
        </h2>
        <p className="relative z-10 text-white/40 text-lg max-w-xl mx-auto mb-10">
          Rejoins la liste d&rsquo;attente. Places limit&eacute;es.
        </p>
        <button
          type="button"
          onClick={() => setFormOpen(true)}
          className="relative z-10 btn-cta btn-shimmer"
        >
          Rejoindre la liste d&rsquo;attente
        </button>
      </section>
