...
import {
  getSectionReport, getSectionRecommendation, getTriggeredInsights, globalKeyInsights, generateTopActions, generateFullReport,
} from '@/lib/bilan-digestif-report'
...
function SectionIntro({ section, sectionIndex, onStart }: { section: TestSection; sectionIndex: number; onStart: () => void }) {
  return (
    <div className="animate-fade-in flex flex-col items-center justify-center min-h-[70vh] text-center px-4">
      <div className="mb-6 inline-flex items-center gap-2 bg-[#1a1a1a]/[0.04] border border-[#1a1a1a]/[0.08] rounded-full px-4 py-1.5">
      ...
      <p className="text-[#2D6A4F] font-medium text-sm tracking-wide uppercase mb-6">{section.subtitle}</p>
      <p className="max-w-md text-[#1a1a1a]/60 leading-relaxed mb-4">{section.description}</p>
      <div className="flex items-center gap-6 mb-10">
        <div className="text-center">
          <p className="text-2xl font-bold text-[#1a1a1a]">{section.tests.length}</p>
          <p className="text-xs text-[#1a1a1a]/40 uppercase tracking-wider">Questions</p>
        </div>
        <div className="w-px h-8 bg-[#1a1a1a]/[0.06]" />
        <div className="text-center">
          <p className="text-2xl font-bold text-[#1a1a1a]">{section.maxScore}</p>
          <p className="text-xs text-[#1a1a1a]/40 uppercase tracking-wider">Points max</p>
        </div>
      </div>
...
function SectionResultsScreen({ section, sectionIndex, scores, onContinue }: {
  ...
  return (
    <div className="animate-fade-in flex flex-col items-center justify-center min-h-[70vh] text-center px-4">
      ...
      <div className="mb-6 inline-flex items-center gap-2 bg-[#1a1a1a]/[0.04] border border-[#1a1a1a]/[0.08] rounded-full px-4 py-1.5">
        <span className="text-xs font-medium tracking-widest uppercase text-[#1a1a1a]/50">Section {sectionIndex + 1} terminée</span>
      </div>
      ...
      <div className="mt-8 max-w-sm">
        ...
        <p className="text-sm text-[#1a1a1a]/60 leading-relaxed">{feedback.message}</p>
      </div>
      <div className="w-full max-w-sm mt-8 space-y-1.5">
        {section.tests.map((test) => {
          ...
          return (
            <div key={test.id} className="flex items-center justify-between bg-[#1a1a1a]/[0.03] border border-[#1a1a1a]/[0.1] rounded-lg px-3 py-2">
              <span className="text-xs text-[#1a1a1a]/70 truncate mr-2">{test.name}</span>
      ...
      {!isLast && (
        <p className="mt-4 text-xs text-[#1a1a1a]/30">
...
function WelcomeScreen({ onStart }: { onStart: () => void }) {
  return (
    <div className="animate-fade-in flex flex-col items-center justify-center min-h-[80vh] text-center px-4">
      ...
      <h1 className="text-3xl md:text-4xl font-bold text-[#1a1a1a] mb-3 tracking-tight">Bilan Digestif</h1>
      <h2 className="text-sm font-medium tracking-widest uppercase text-[#2D6A4F] mb-8">Troubles & confort digestif</h2>
      ...
      <div className="w-full max-w-sm space-y-3 mb-10">
        {allSections.map((section) => (
          <div key={section.id} className="flex items-center gap-4 bg-[#1a1a1a]/[0.03] border border-[#1a1a1a]/[0.1] rounded-xl px-4 py-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#2D6A4F]/10 to-[#1B4332]/10 border border-[#2D6A4F]/10 flex items-center justify-center text-[#2D6A4F]/70">
              {renderSectionIcon(section.icon, 'w-5 h-5')}
            </div>
            <div className="flex-1 text-left">
              <p className="text-sm font-semibold text-[#1a1a1a]">{section.title}</p>
              <p className="text-xs text-[#1a1a1a]/40">{section.tests.length} questions</p>
            </div>
            <span className="text-xs tabular-nums text-[#1a1a1a]/30">{section.maxScore} pts</span>
          </div>
        ))}
      </div>
      <button onClick={onStart} className="btn-secondary">Commencer le bilan</button>
      <p className="mt-6 text-xs text-[#1a1a1a]/30 max-w-xs leading-relaxed">
  ...
  return (
    <div className="animate-fade-in max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="bg-white backdrop-blur-sm border border-[#1a1a1a]/[0.08] rounded-2xl p-8 mb-8 text-center">
        <p className="text-xs font-medium tracking-widest uppercase text-[#1a1a1a]/40 mb-4">Score global</p>
      ...
      <div className="space-y-3 mb-10">
        {sectionResults.map(({ section, score }) => {
          ...
          const info = getOverallLabel(pct)
          return (
            <div key={section.id} className="bg-white border border-[#1a1a1a]/[0.1] rounded-xl px-5 py-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-[#1a1a1a]/40">{score}/{section.maxScore}</p>
                </div>
              </div>
        ...
        <div className="animate-fade-in">
          ...
          {interpretation && (
            <div className="bg-white border border-[#1a1a1a]/[0.1] rounded-2xl p-6 mb-8">
              <p className="text-sm text-[#1a1a1a]/70 leading-relaxed mb-3">{interpretation.description}</p>
              <div className="flex items-start gap-2 pt-3 border-t border-[#1a1a1a]/[0.1]">
                ...
                <p className="text-xs text-[#1a1a1a]/60 leading-relaxed">{interpretation.recommendation}</p>
              </div>
            </div>
          )}
        ...
        <div className="sticky top-0 z-30 bg-[#FAF8F5]/95 backdrop-blur-md border-b border-[#1a1a1a]/[0.1]">
          <div className="max-w-lg mx-auto px-4 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs tabular-nums text-[#1a1a1a]/40">{flatIndex + 1}/{totalTests}</span>
