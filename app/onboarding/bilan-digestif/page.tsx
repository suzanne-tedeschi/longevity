...
function DigestifIcon({ className = 'w-6 h-6' }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div className="w-full mb-8">
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-medium tracking-widest uppercase text-white/60">{sectionTitle}</span>
        <span className="text-xs tabular-nums text-white/50">{current}/{total}</span>
      </div>
      <div className="h-1 w-full bg-white/[0.1] rounded-full overflow-hidden">
        <div className="h-full rounded-full bg-gradient-to-r from-[#c9a96e] to-[#a08050] transition-all duration-700 ease-out" style={{ width: `${pct}%` }} />
      </div>
    </div>
  )
}
    ...
    <button
      ...
      className={`group relative w-full text-left rounded-xl border-2 p-4 transition-all duration-300 ease-out ${
        selected
          ...
          : 'border-white/[0.1] bg-white/[0.05] hover:border-white/20 hover:shadow-sm'
      }`}
    >
      <div className="flex items-start gap-3">
        <div className={`flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center text-sm font-bold transition-all duration-300 ${
          selected ? `${colors.bg} ${colors.text} ${colors.border} border` : 'bg-white/[0.06] text-white/40 border border-white/[0.1]'
        }`}>
          {value}
        </div>
        <div className="flex-1 min-w-0">
          <p className={`font-semibold text-sm transition-colors duration-300 ${selected ? colors.text : 'text-white'}`}>{label}</p>
...
function SectionIntro({ section, sectionIndex, onStart }: { section: TestSection; sectionIndex: number; onStart: () => void }) {
  return (
    <div className="animate-fade-in flex flex-col items-center justify-center min-h-[70vh] text-center px-4">
      <div className="mb-6 inline-flex items-center gap-2 bg-white/[0.06] border border-white/[0.08] rounded-full px-4 py-1.5">
        <span className="text-xs font-medium tracking-widest uppercase text-white/50">
          Section {sectionIndex + 1} / {allSections.length}
        </span>
      </div>
      <div className="mb-8 w-20 h-20 rounded-2xl bg-gradient-to-br from-[#c9a96e]/10 to-[#a08050]/10 border border-[#c9a96e]/15 flex items-center justify-center text-[#c9a96e]">
      ...
      <p className="text-[#c9a96e] font-medium text-sm tracking-wide uppercase mb-6">{section.subtitle}</p>
      <p className="max-w-md text-white/60 leading-relaxed mb-4">{section.description}</p>
      <div className="flex items-center gap-6 mb-10">
        <div className="text-center">
          <p className="text-2xl font-bold text-white">{section.tests.length}</p>
          <p className="text-xs text-white/40 uppercase tracking-wider">Questions</p>
        </div>
        <div className="w-px h-8 bg-white/[0.1]" />
        <div className="text-center">
          <p className="text-2xl font-bold text-white">{section.maxScore}</p>
          <p className="text-xs text-white/40 uppercase tracking-wider">Points max</p>
        </div>
      </div>
...
function TestCard({ test, testIndex, totalTests, sectionTitle, sectionIcon, selectedScore, onScore, onPrev, onNext, canGoNext }: {
  test: DigestifTest; testIndex: number; totalTests: number; sectionTitle: string; sectionIcon: SectionIcon
    ...
    <div className="animate-fade-in max-w-lg mx-auto">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-3">
          <div className="w-7 h-7 rounded-lg bg-[#c9a96e]/10 flex items-center justify-center text-[#c9a96e]">
            {renderSectionIcon(sectionIcon, 'w-4 h-4')}
          </div>
          <span className="text-xs font-medium tracking-widest uppercase text-white/40">Question {testIndex + 1}</span>
        </div>
        <h3 className="text-2xl font-bold text-white tracking-tight mb-1">{test.name}</h3>
        <p className="text-sm text-[#c9a96e] font-medium">{test.description}</p>
      </div>

      <div className="bg-white/[0.04] border border-white/[0.1] rounded-xl p-4 mb-6">
        <p className="text-sm text-white/70 leading-relaxed">{test.criteria}</p>
        {test.tip && (
          <div className="flex items-start gap-2 mt-3 pt-3 border-t border-white/[0.1]">
            <InfoIcon className="w-4 h-4 text-[#c9a96e] flex-shrink-0 mt-0.5" />
            <p className="text-xs text-white/50 leading-relaxed">{test.tip}</p>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between">
        <button onClick={onPrev} className="flex items-center gap-1 text-sm text-white/50 hover:text-white transition-colors">
        ...
        <button
          ...
          className={`flex items-center gap-1 px-6 py-2.5 rounded-xl text-sm font-semibold transition-all duration-300 ${
            canGoNext
              ? 'bg-[#c9a96e] text-white shadow-md hover:shadow-lg hover:-translate-y-0.5'
    ...
    <div className="flex flex-col items-center">
      <div className={`relative ${dims.box}`}>
        <svg className="w-full h-full -rotate-90" viewBox={`0 0 ${(dims.r + dims.stroke) * 2} ${(dims.r + dims.stroke) * 2}`}>
          <circle cx={dims.r + dims.stroke} cy={dims.r + dims.stroke} r={dims.r} fill="none" stroke="#2a2a2a" strokeWidth={dims.stroke} />
          <circle
            ...
            style={{ color: pct >= 90 ? '#10b981' : pct >= 75 ? '#0ea5e9' : pct >= 50 ? '#f59e0b' : pct >= 30 ? '#f97316' : '#ef4444' }}
          />
        </svg>
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className={`text-2xl font-bold ${color}`}>{pct}%</span>
          <span className="text-[10px] text-white/40 mt-0.5">{score}/{maxScore}</span>
        </div>
      </div>
...
function getSectionFeedback(sectionId: string, pct: number): { title: string; message: string } {
  ...
  const feedback: Record<string, Record<string, { title: string; message: string }>> = {
    'reflux': {
      high: { title: 'Pas de reflux significatif', message: 'Vous ne présentez pas de symptômes de reflux gastro-œsophagien. C\u2019est un bon signe pour votre confort digestif haut.' },
      mid:  { title: 'Reflux occasionnels', message: 'Vous ressentez parfois des brûlures ou remontées acides. Des ajustements alimentaires et posturaux peuvent aider.' },
      low:  { title: 'Reflux fréquent', message: 'Le reflux est un problème régulier pour vous. Un bilan avec votre médecin et un programme nutritionnel adapté sont recommandés.' },
    },
    'douleurs-abdominales': {
      high: { title: 'Abdomen confortable', message: 'Vous ne ressentez pas de douleurs abdominales significatives. Votre système digestif semble bien fonctionner.' },
      mid:  { title: 'Gênes abdominales modérées', message: 'Certaines douleurs ou gênes nécessitent attention. Le programme intègrera des recommandations ciblées.' },
      low:  { title: 'Douleurs fréquentes', message: 'Les douleurs abdominales impactent votre quotidien. Un accompagnement médical et nutritionnel est fortement conseillé.' },
    },
    'indigestion': {
      high: { title: 'Bonne digestion', message: 'Vos fonctions digestives semblent bien équilibrées. Peu de ballonnements ou de gaz.' },
      mid:  { title: 'Digestion à optimiser', message: 'Quelques symptômes de ballonnements ou de gaz perturbent votre confort. Des pistes nutritionnelles existent.' },
      low:  { title: 'Difficultés digestives', message: 'Les ballonnements et gaz sont un problème récurrent. Un travail sur le microbiote et l\u2019alimentation sera prioritaire.' },
    },
    'diarrhee': {
      high: { title: 'Transit stable', message: 'Votre transit n\u2019est pas accéléré. C\u2019est un bon indicateur de santé intestinale.' },
      mid:  { title: 'Transit parfois accéléré', message: 'Des épisodes de diarrhée ou d\u2019urgence surviennent occasionnellement. Le programme adaptera vos apports.' },
      low:  { title: 'Transit accéléré fréquent', message: 'La diarrhée ou les urgences intestinales sont un problème régulier. Un bilan approfondi est recommandé.' },
    },
    'constipation': {
      high: { title: 'Pas de constipation', message: 'Votre transit est régulier et confortable. Continuez à maintenir une bonne hydratation et un apport en fibres.' },
      mid:  { title: 'Constipation occasionnelle', message: 'Vous rencontrez parfois des difficultés de transit. Des ajustements simples peuvent améliorer la situation.' },
      low:  { title: 'Constipation fréquente', message: 'La constipation est un problème récurrent. Le programme inclura des recommandations spécifiques pour le transit.' },
    },
  }
...
function SectionResultsScreen({ section, sectionIndex, scores, onContinue }: {
  ...
  return (
    <div className="animate-fade-in flex flex-col items-center justify-center min-h-[70vh] text-center px-4">
      ...
      <div className="mb-6 inline-flex items-center gap-2 bg-white/[0.06] border border-white/[0.08] rounded-full px-4 py-1.5">
        <span className="text-xs font-medium tracking-widest uppercase text-white/50">Section {sectionIndex + 1} terminée</span>
      </div>
      ...
      <div className="mt-8 max-w-sm">
        ...
        <p className="text-sm text-white/60 leading-relaxed">{feedback.message}</p>
      </div>
      <div className="w-full max-w-sm mt-8 space-y-1.5">
        {section.tests.map((test) => {
          ...
          return (
            <div key={test.id} className="flex items-center justify-between bg-white/[0.04] border border-white/[0.1] rounded-lg px-3 py-2">
              <span className="text-xs text-white/70 truncate mr-2">{test.name}</span>
...
function WelcomeScreen({ onStart }: { onStart: () => void }) {
  return (
    <div className="animate-fade-in flex flex-col items-center justify-center min-h-[80vh] text-center px-4">
      ...
      <div className="mb-8 w-16 h-16 rounded-2xl bg-gradient-to-br from-[#c9a96e]/10 to-[#a08050]/10 border border-[#c9a96e]/15 flex items-center justify-center text-[#c9a96e]">
        <DigestifIcon className="w-8 h-8" />
      </div>
      <h1 className="text-3xl md:text-4xl font-bold text-white mb-3 tracking-tight">Bilan Digestif</h1>
      <h2 className="text-sm font-medium tracking-widest uppercase text-[#c9a96e] mb-8">Troubles & confort digestif</h2>
      <p className="max-w-md text-white/60 leading-relaxed mb-10">
        Ce bilan évalue vos symptômes digestifs et votre confort intestinal à travers
        {' '}{allSections.reduce((sum, s) => sum + s.tests.length, 0)} questions réparties en {allSections.length} sections.
        Il est basé sur le questionnaire GSRS (Gastrointestinal Symptom Rating Scale), un outil validé scientifiquement.
      </p>
      <div className="w-full max-w-sm space-y-3 mb-10">
        {allSections.map((section) => (
          <div key={section.id} className="flex items-center gap-4 bg-white/[0.04] border border-white/[0.1] rounded-xl px-4 py-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#c9a96e]/10 to-[#a08050]/10 border border-[#c9a96e]/10 flex items-center justify-center text-[#c9a96e]/70">
              {renderSectionIcon(section.icon, 'w-5 h-5')}
            </div>
            <div className="flex-1 text-left">
              <p className="text-sm font-semibold text-white">{section.title}</p>
              <p className="text-xs text-white/40">{section.tests.length} questions</p>
            </div>
            <span className="text-xs tabular-nums text-white/30">{section.maxScore} pts</span>
          </div>
        ))}
      </div>
      <button onClick={onStart} className="btn-secondary">Commencer le bilan</button>
      <p className="mt-6 text-xs text-white/30 max-w-xs leading-relaxed">
        Répondez en pensant à la dernière semaine écoulée. Soyez le plus honnête possible.
      </p>
    </div>
  )
}
...
function ResultsScreen({ scores }: { scores: Record<string, number> }) {
  ...
  const interpretation = digestifInterpretations.find((interp, i) => {
    ...
    if (totalPct >= 75 && totalPct < 90 && i === 1) return true
    if (totalPct >= 50 && totalPct < 75 && i === 2) return true
    if (totalPct >= 30 && totalPct < 50 && i === 3) return true
    if (totalPct < 30 && i === 4) return true
    return false
  })
  ...
  return (
    <div className="animate-fade-in max-w-2xl mx-auto px-4 py-8">
      <div className="text-center mb-12">
        ...
        <h2 className="text-3xl md:text-4xl font-bold text-white tracking-tight mb-2">Vos résultats</h2>
        <p className="text-sm text-white/50">Bilan digestif — symptômes & confort</p>
      </div>

      {/* Global score */}
      <div className="bg-white/[0.05] backdrop-blur-sm border border-white/[0.08] rounded-2xl p-8 mb-8 text-center">
        <p className="text-xs font-medium tracking-widest uppercase text-white/40 mb-4">Score global</p>
        ...
        <p className="text-sm text-white/50 mb-3">{totalScore} / {totalMaxScore} points</p>
        <span className={`inline-block px-4 py-1 rounded-full text-xs font-semibold tracking-wide ${overall.color} bg-white/[0.06]`}>
          {overall.label}
        </span>
      </div>
      ...
      {/* Interpretation */}
      {interpretation && (
        <div className="bg-white/[0.05] border border-white/[0.1] rounded-2xl p-6 mb-8">
          <p className="text-sm text-white/70 leading-relaxed mb-3">{interpretation.description}</p>
          <div className="flex items-start gap-2 pt-3 border-t border-white/[0.1]">
            <InfoIcon className="w-4 h-4 text-[#c9a96e] flex-shrink-0 mt-0.5" />
            <p className="text-xs text-white/60 leading-relaxed">{interpretation.recommendation}</p>
          </div>
        </div>
      )}

      {/* Section gauges */}
      <div className="grid grid-cols-3 sm:grid-cols-5 gap-6 mb-10">
      ...
      <div className="space-y-4 mb-10">
        {sectionResults.map(({ section, score }) => {
          ...
          const info = getOverallLabel(pct)
          return (
            <div key={section.id} className="bg-white/[0.05] border border-white/[0.1] rounded-xl px-5 py-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-[#c9a96e]/10 flex items-center justify-center text-[#c9a96e]/70">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-white/40">{score}/{section.maxScore}</p>
                </div>
              </div>
...
export default function BilanDigestifPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
      {phase === 'testing' && (
        <div className="sticky top-0 z-30 bg-[#0a0a0a]/90 backdrop-blur-md border-b border-white/[0.1]">
          <div className="max-w-lg mx-auto px-4 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs tabular-nums text-white/40">{flatIndex + 1}/{totalTests}</span>
            ...
            <Link href="/onboarding/bilans" className="text-xs text-white/30 hover:text-white/60 transition-colors">Quitter</Link>
          </div>
        </div>
      )}
